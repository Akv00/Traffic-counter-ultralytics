...
from ultralytics.yolo.utils import DEFAULT_CONFIG, LOGGER, SETTINGS, ops
...
class DetectionPredictor():
    def __init__(self, source="test.mp4",model="yolov8n.pt"):
        self.args = get_config(DEFAULT_CONFIG, {})
        self.dataset = None
        self.source = source
        self.model = model
    ...
    def write_results(self,idx, preds, batch):
        ...
        log_string += "count: " + str(object_counter) + " count1: " + str(object_counter1)
        return log_string
    ...
    def setup(self):
        ...
        bs = 1
        self.dataset = LoadImages(self.source,
                                  imgsz=imgsz,
                                  stride=stride,
                                  auto=pt,
                                  transforms=getattr(
                                      model.model, 'transforms', None),
                                  vid_stride=self.args.vid_stride)
        # self.frame_width = self.dataset.cap.get(3)
        # self.frame_height = self.dataset.cap.get(4)
        codec = cv2.VideoWriter_fourcc('X','V','I','D')
        fps = 30
        size = (1280,720)
        self.videoWriter = cv2.VideoWriter("output.avi",codec,fps,size)
    ...
    def call(self):
        ...
        for batch in self.dataset:
            ...
            # Inference
            with self.dt[1]:
                preds = model(im, augment=self.args.augment)
